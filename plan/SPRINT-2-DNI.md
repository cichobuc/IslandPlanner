...
- [x] Vercel: https://islandplanner.vercel.app (import z GitHubu, 20. 9. 2026) · [ ] Supabase Free projekt (login GitHub) – pošli mi `SUPABASE_URL`, `ANON_KEY`, `SERVICE_ROLE_KEY`, DB heslo
...
| 1.4 | 2:45–3:45 | `/profil` – formulár podľa 00a, uloženie, prenos do cestujúceho; sekcia „Kedy môžem" doplnená 21. 9. (mesiace, blokované termíny, dĺžka → prienik skupiny v kroku 01 + kalendár v 02) | profil uložený, vidno v DB |
...
| 2.6 | 5:15–6:15 | **krok Kde spať** (od ADR-014 = 05): noci s regiónmi z presetu, izby = rozpätia zo seed tabuľky, kempy z tjalda (živé ceny, otvorenie), ručné vloženie ponuky, kuchynka → strava | 9 nocí s rozpätím / kemp cenou |
| 2.7 | 6:15–7:30 | **krok Itinerár (od ADR-014 = 04) + Mapa**: `route_matrix` pre 20 POI (skript cez ORS, commit), preset Ring/Juh, dni → zastávky (vstupné podľa veku), **reálna OSM mapa** (MapLibre + OpenFreeMap) s trasou a nocami – body cesty s **tooltipom pri hoveri**, klik = otvorí sheet s detailom / rozbalí riadok (požiadavka 20. 9.); **krok 06** ako zoznam v pláne + katalóg zo seedu | trasa na OSM mape, body klikateľné, vstupné pre 4 |
